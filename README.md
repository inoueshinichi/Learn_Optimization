...
\end{eqnarray}
```
+ 関数yに対するパラメータを $\boldsymbol{t},\boldsymbol{x}$ とするとき, $\frac{dy}{d\boldsymbol{x}}$ , $\frac{dy}{d\boldsymbol{t}}=\frac{dy}{d\boldsymbol{x}}*\frac{d\boldsymbol{x}}{d\boldsymbol{t}}$ と各パラメータの微分値(勾配)を求めることをバックプロパゲーション法と呼ぶ.
